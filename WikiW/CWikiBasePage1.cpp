...
void CWikiBasePage1::CreateChildControls ( void )
{
	...
	b_Refresh.Create ( L"Обновить", WS_CHILD | WS_VISIBLE | BS_OWNERDRAW, CRect ( 320, 250, 520, 290 ), this, IDC_PANEL_MAIN_REFRESH );
...
void CWikiBasePage1::OnRefreshClick ( void )
{
	if ( !isConnected )
	{
		...
		//api_url = "/w/api.php?action=parse&format=json&page=Семнадцать мгновений весны (телефильм)";
